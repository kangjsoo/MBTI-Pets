...
const db = {
  ...
  async clearAllSubmissions(userId) {
    ...
    // Use a batch for multiple deletes
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50 font-sans">
      <header className="p-4 flex justify-between items-center">
        <h1 className="text-2xl font-bold text-purple-800 cursor-pointer" onClick={resetApp}>멍BTI 테스트</h1>
        <button onClick={handleAdminLogin} className="text-sm text-gray-500 hover:text-purple-600">관리자</button>
      </header>
      <main className="max-w-2xl mx-auto p-4 md:p-8">
        {renderPage()}
      </main>
      {showAdminModal && (
        <CustomModal onClose={() => setShowAdminModal(false)}>
            {modalContent}
        </CustomModal>
      )}
      {showConfirmModal && (
        <CustomModal onClose={() => setShowConfirmModal(false)}>
            {modalContent}
        </CustomModal>
      )}
      <footer className="text-center text-gray-400 p-4 text-xs">
          © 2024 My Pet Project. All rights reserved.
      </footer>
    </div>
  );
}
